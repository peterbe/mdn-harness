...
async function savePayload(payload, resourceUrl, folder, logger) {
  const pathname = url.parse(resourceUrl).pathname.slice(1).split("?")[0];
...
prog
  ...
  .action(async function (args, options, logger) {
    ...
    const html = await downloadHTML(args.url, destination, logger);
    const assetsDownloaded = await downloadAssets(
      args.url,
      destination,
      logger
    );
    ...
    function getDownloadedAsset(uri) {
      const absoluteUri = new URL(uri, args.url).toString();
    ...
    $("script,link,img").each((i, element) => {
      const $element = $(element);
      let uri;
      if (element.tagName === "script" || element.tagName === "image") {
        ...
        logger.warn(`DON'T KNOW HOW TO DEAL WITH '${element.tagName}'!`);
    ...
    $("iframe").each((i, element) => {
      ...
      if (savedPath) {
        $element.attr("src", savedPath);
        logger.info(`Changed HTML for iframe ${uri}`);
      }
    });
    ...
    logger.info(`You might now want to run:
    serve -s ${destination}
    `);
  });
